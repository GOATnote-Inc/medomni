...
**Training pipeline is now structurally unblocked.** V2.5 reasoning-SFT can fire per its PREREG (`findings/2026-05-05-v2.5-reasoning-sft/PREREG.yaml`) — base model is on disk, HF auth works, disk has 58 GB headroom for V2.5 LoRA + intermediate checkpoints (~10-15 GB needed). Babysitter cannot fire training per harmony contract — that's user-action. Once user issues training go-ahead, the chain V2.5 → V2.7 → V3 → V3.5 → V_final per existing PREREGs runs.

**iter-46 update (2026-05-05 ~13:25 PT): FP8/BF16 base-precision MISMATCH found + remediated.** Pre-flight check before declaring V2.5 ready surfaced that `train_peft_imaging.py:455-465` hard-codes `_check_bf16_only` ("RULES §3: bf16-mixed only. Refuse fp8/NVFP4."), making the iter-45 FP8 download incompatible with the bf16-mixed PEFT trainer. See `findings/2026-05-05-v2.5-base-precision-mismatch/CARD.md` for full analysis. User authorized Option A ("ok ultrathink"); babysitter dropped FP8 (33 GB recovered) and fired BF16 download (PID 2420073, 50 files, ~67.75 GB target). In progress at commit time. Final disk projected at ~22 GB free post-download. Durable lesson saved: pre-flight check the actual trainer's accepted dtypes before recommending non-default base precision; "research papers say X is fine" ≠ "this codebase's trainer accepts X". V2.5 still gated on BF16 download completion (iter-47 verification).
