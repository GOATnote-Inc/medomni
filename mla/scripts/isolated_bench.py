...
class Grid:
    ...
    def iter_points(self):
        for subj, b, k, d in itertools.product(
            self.subjects, self.batches, self.kv_lens, self.dtypes
        ):
        ...
        summary.append(
            {
                ...
                "cold_ns_mean": (
                    statistics.fmean([g.get("cold_ns", 0) for g in ok]) if ok else None
                ),
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", default=None, help="output dir (default: results/logs/)")
    args = ap.parse_args()
    ...
    print(
        f"[plan] {total} runs across subjects={args.subjects}  "
