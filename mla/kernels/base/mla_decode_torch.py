...
def make_torch_inputs(cfg: TorchMLAConfig, *, seed: int = 0) -> dict[str, Any]:
    ...
    q_nope = torch.randn(
        cfg.batch, cfg.heads, cfg.d_ckv, dtype=torch.float32, generator=gen, device=device
    ).to(dt)
    q_pe = torch.randn(
        cfg.batch, cfg.heads, cfg.d_pe, dtype=torch.float32, generator=gen, device=device
    ).to(dt)
    ckv = torch.randn(
        cfg.batch, cfg.kv_len, cfg.d_ckv, dtype=torch.float32, generator=gen, device=device
    ).to(dt)
    kpe = torch.randn(
        cfg.batch, cfg.kv_len, cfg.d_pe, dtype=torch.float32, generator=gen, device=device
    ).to(dt)
    return {
        "q_nope": q_nope,
        "q_pe": q_pe,
