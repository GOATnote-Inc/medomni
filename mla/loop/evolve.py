    ...
    if isinstance(base_sc, ScoredCandidate):
        for i in islands:
            i.members[0] = ScoredCandidate(
                ...
                score=base_sc.score,
                tokens_per_sec=base_sc.tokens_per_sec,
                stability=base_sc.stability,
                median_ns=base_sc.median_ns,
        ...
        for island in islands:
            ...
            if cfg.pareto_keep and len(scored) > 1:
                ...
                extras = [
                    s
                    for s in scored
                    if s.candidate.source_hash in front_ids
        ...
        if it % cfg.migrate_every == 0:
            ...
            if best and best not in weakest.members:
                ...
                it_record["migrations"].append(
                    {
                        "from": best.candidate.island,
                        "to": weakest.name,
                        "hash": best.candidate.source_hash,
                        "score": best.score,
                    }
                )
