...
def _flashinfer_reference_benchmark(
    cfg: TorchMLAConfig, kv_lens: list[int]
) -> list[dict[str, Any]]:
        ...
        if not v.passed:
            scored.append(
                TorchScore(
                    name=cand.name,
                    passed=False,
                    reason=v.failed_check or "unknown",
...
def main() -> int:
    ...
    p.add_argument(
        "--sweep", default="256,4096", help="comma-separated extra kv_lens for Tier-2 sweep"
    )
    args = p.parse_args()
