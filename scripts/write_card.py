...
def _ci_overlap(a_mean: float, a_hw: float, b_mean: float, b_hw: float) -> tuple[bool, str]:
...
def _load_latest_manifest() -> dict:
    """Read the newest reproducibility/captured/manifest-*.yaml so the CARD's
    Provenance section reflects the actual run hardware/container, not stale
    hardcoded H200 strings. Returns {} if none found or YAML unavailable."""
    captured = Path(__file__).resolve().parent.parent / "reproducibility" / "captured"
    if not captured.is_dir():
        return {}
    candidates = sorted(captured.glob("manifest-*.yaml"), key=lambda p: p.stat().st_mtime)
    if not candidates:
        return {}
    latest = candidates[-1]
    try:
        ...
        try:
            ...
            # _yaml_dump fallback writes JSON-as-YAML with a header line.
            for prefix in ("# Generated by scripts/freeze_snapshot.py (JSON-as-YAML fallback)\n",):
...
def main() -> int:
    ...
    out_path = Path(args.out).resolve() if args.out else artifact_path.parent / "CARD.md"
    ...
    a("")
    a(
        f"- **Score**: `{mean:.3f} ± {hw:.3f}` (mean ± 95% half-width across N={n_trials} trial aggregates)"
    )
    ...
    bare = (manifest.get("bare_metal") or {}) if manifest else {}
    inf = (manifest.get("inference") or {}) if manifest else {}
    gpu_type = bare.get("gpu_type") or "unknown"
    driver = bare.get("driver_version") or "unknown"
    container_tag = inf.get("serve_image_resolved_tag") or "unknown"

    a(
        f"| **{payload.get('serve_model', 'this run')}** (sovereign, {gpu_type}, this run) | `{mean:.3f} ± {hw:.3f}` | {n_trials} | {payload.get('generated_at', '?')[:10]} |"
    )
    ...
    a("")
    a(
        f"- Serve URL : `{payload.get('serve_url', '?')}` (localhost-only enforced in `mla/judges/triton.py`)"
    )
    ...
    a("")
    a(
        f"- **Judge incompleteness**: {judge_incomplete_total} rubric items recused across "
    ...
    a(
        "- **Same-family judge bias**: serve and judge are the same Nemotron-3-Nano-30B-A3B-BF16 "
    ...
    a(
        "- **Non-paired comparison**: the Opus 4.7 baseline was measured on a different day with a "
    ...
    a(f"- Hardware: {gpu_type}, driver {driver}.")
    a(f"- Container: `{container_tag}`.")
    temp = payload.get("temperature")
    sysp = payload.get("clinical_system_prompt")
    if temp is not None or sysp is not None:
        a(
            f"- Run knobs: temperature={temp if temp is not None else '?'}, "
            f"clinical_system_prompt={sysp if sysp is not None else '?'}, enable_thinking=False."
        )
    a("")
