...
def main() -> int:
    ...
    a(f"| **{payload.get('serve_model', 'this run')}** (sovereign, {gpu_type}, this run) | `{mean:.3f} ± {hw:.3f}` | {n_trials} | {payload.get('generated_at', '?')[:10]} |")
    ...
    if temp is not None or sysp is not None:
        a(f"- Run knobs: temperature={temp if temp is not None else '?'}, "
